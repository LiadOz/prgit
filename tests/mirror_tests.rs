...
#[test]
fn test_ktext_file_stores_unexpanded_keywords() {
    let env = MirrorTestEnv::new();
    let content = "# Script\n$Id$\n$Author$\nprint('hello')\n";
    env.p4_client
        ...
        .add_file_with_opts(
            "rbs_sql.py",
            content.as_bytes(),
            Some(FileType::text().keyword_expansion()),
        )
        .submit()
        .expect("submit failed");
    ...
    env.mirror().run().expect("Mirror failed");
    let git_files = env.get_git_files();
    let git_content = git_files.get("rbs_sql.py").expect("file should exist in git");
    let git_str = String::from_utf8(git_content.clone()).expect("should be utf8");

    assert!(
        git_str.contains("$Id$"),
        "Git should store unexpanded $Id keyword, got: {}",
        git_str
    );
    assert!(
        git_str.contains("$Author$"),
        "Git should store unexpanded $Author keyword, got: {}",
        git_str
    );
    assert!(
        !git_str.contains("$Id: "),
        "Git should NOT contain expanded $Id keyword, got: {}",
        git_str
    );
}

#[test]
fn test_ktext_file_edit_preserves_unexpanded_keywords() {
    let env = MirrorTestEnv::new();
    let content_v1 = "$Id$\nversion 1\n";
    env.p4_client
        .changelist("Add ktext")
        .add_file_with_opts(
            "versioned.py",
            content_v1.as_bytes(),
            Some(FileType::text().keyword_expansion()),
    ...
    let content_v2 = "$Id$\nversion 2\n";
    env.p4_client
        .changelist("Edit ktext")
        .edit_file("versioned.py", content_v2)
        .submit()
        .expect("submit failed");

    env.mirror().run().expect("Mirror failed after edit");
    let git_files = env.get_git_files();
    let git_content = git_files
        .get("versioned.py")
        .expect("file should exist in git");
    let git_str = String::from_utf8(git_content.clone()).expect("should be utf8");

    assert_eq!(
        git_str, content_v2,
        "After edit, git should have unexpanded v2 content"
    );
}
