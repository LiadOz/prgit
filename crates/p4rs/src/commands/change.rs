...
use crate::commands::types::{ChangeStatus, deserialize_p4_date};
...
impl<'p, 's> P4Command for ChangeCommand<'p, &'s ChangeSpec> {
    ...
    fn run(&self) -> Result<Self::Response, P4Error> {
        ...
        let change: usize = result.split_whitespace()
            .nth(1)
            .ok_or(P4Error::UnexpectedError(format!("unexpected output: {}", result)))?
            .parse()
